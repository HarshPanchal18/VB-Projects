...
Public Class Booking
    Private Sub Btnload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btnload.Click, BtnAdd.Click
        dbdt = New DataTable
        Try
    ...
    Private Sub BtnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAdd.Click
        Try
            conn.Open()
            Dim qry As String
            qry = "insert into tblrentcar values(" & TBoxID.Text & ",'" & Cboxcustomer.Text & "','" & Cboxcarname.Text & "','" & Txtboxplate.Text & "','" & DateTimePicker1.Text & "'," & Txtrent.Text & "," & Txtnodays.Text & "," & Txtotal.Text & ",'" & Txtprocess.Text & "')"
            With dbcmd
                .CommandText = qry
                .Connection = conn
            End With
            ...
            If result > 0 Then
                MsgBox("New carrent record has been inserted!")
                conn.Close()
                Call Btnload_Click(sender, e)
                cleartextfields()
            Else
                MsgBox("No carrent record has been inserted!")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
    ...
    Private Sub Btnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btnsave.Click
        Try
            ...
            result = dbcmd.ExecuteNonQuery
            If result > 0 Then
                MsgBox("New car-rent record has been saved!")
                conn.Close()
                Call Btnload_Click(sender, e)
                cleartextfields()
            Else
                MsgBox("No car-rent record has been saved!!")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
